...
export interface IEpiserverSpaContext {
     ...
     */
    getContentByGuid(guid: string) : IContent
     ...
     */
    getContentById(id: ContentApiId) : IContent
    ...
    getContentByRef(ref: string) : IContent
    loadContentByRef(ref: string) : Promise<IContent>
    getContentByPath(path: string) : IContent
...
export class EpiserverSpaContext implements IEpiserverSpaContext {
    protected _config : Readonly<EpiConfig>;
    protected _initialized : boolean = false;
    protected _state : EnhancedStore = null;
    protected _contentApi: ContentDeliveryAPI = null;
    protected _isServerSideRendering: boolean = null;
    protected _componentLoader: ComponentLoader = null;
    ...
    public isDebugActive() : boolean
    {
        ...
        return this._config ? this._config.enableDebug : false;
    }
    ...
    public getContentByGuid(guid: string) : IContent
    {
    ...
    {
        let c : IContent = this.getContentByGuid(id);
        if (c) {
    ...
    public getContentById(id: ContentApiId) : IContent
    {
    ...
    {
        let c : IContent = this.getContentById(id);
        if (c) {
            return Promise.resolve(c);
        }
        return this.invoke(IContentRepository.getById(id));
    ...
    public getContentByRef(ref: string) : IContent
    {
    ...
    public getContentByPath(path: string) : IContent
    {
    ...
    {
        let c : IContent = this.getContentByPath(path);
        if (c) {
            return Promise.resolve(c);
        }
    ...
    public navigateTo(path: ContentReference, noHistory: boolean = false) : void
    {
        ...
        this.loadContentByPath(newPath).then(() => {
            ...
            if (!noHistory) { History.pushPath(newPath); }
        });
    }
    ...
    {
        return this.getContentByPath(this.getCurrentPath());
    }
