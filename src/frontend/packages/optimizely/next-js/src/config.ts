 ...
 */
 export function withOptimizelyConfig(config: NextConfig, websiteConfig: WebsiteDefinition) : NextConfig
 {
     ...
     for (const key in process?.env ?? {}) if (key.startsWith("OPTIMIZELY_")) 
         config.env[key] = process.env[key] as string
